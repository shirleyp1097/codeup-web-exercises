...
function Employee(firstName, lastName, salesUnits) {
    this.firstName = firstName;
    this.lastName = lastName;
    this.salesUnit = salesUnits;
}

salesReport.employees.push(new Employee('Jane', 'Janeway', 3), new Employee('Tricia', 'Triciason', 5), new Employee('Jeannette', 'Jeanson', 4));
...
salesReport.getTotalNumberOfSales = function() {
    ...
    salesReport.employees.forEach(function(employee, index) {
        TotalNumberOfSales += employee.salesUnit
    })
    return TotalNumberOfSales;
}

// salesReport.getAverageSalesPerEmployee = function() {
//     var AverageSalesPerEmployee = 0
//     for (i=0; i<= salesReport.employees.length; i++) {
//         AverageSalesPerEmployee += salesReport.employees[i].salesUnit
//     }
...
// profileReport.getProfileCount = function() {
//     return profileReport.profiles.length
// };
...
profileReport.getActiveCount = function() {
    let activeProfiles = 0;
    profileReport.profiles.forEach(function(profile) {
...
profileReport.getInactiveCount = function() {
    let activeProfiles = 0;
    profileReport.profiles.forEach(function(profile) {
...
profileReport.sumOfAllBalances = function() {
    let sumOfAllBalances = 0;
    this.profiles.forEach(function(profile) {
        sumOfAllBalances += convert(profile.balance)
    })
    return sumOfAllBalances;
}

profileReport.getAverageBalance = function() {
    let averageBalance = 0;
    this.profiles.forEach(function(profile) {
        averageBalance += convert(profile.balance);
    })
    averageBalance = averageBalance / this.profiles.length
    return averageBalance;
}

profileReport.getHighestBalance = function() {
    let highestBalance = 0;
    this.profiles.forEach(function(profile) {
        if (highestBalance < Math.max(convert(profile.balance))) {
            highestBalance = Math.max(convert(profile.balance))
        }
    })
    return highestBalance;
}

// profileReport.getMostFavoriteFruit = function() {
//     let favoriteFruits = {};
//     for (let i in profiles) {
//         var theFruit = profiles[i].favoriteFruit
//         console.log(theFruit);
//         // if (!(profiles.favoriteFruit in favoriteFruits)) {
//         //     favoriteFruits['']
//         }
//     }
// }
...
console.log(mostFrequent(favoriteFruits))